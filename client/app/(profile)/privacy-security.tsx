...
function PrivacyAndSecurityScreen() {
  ...
  return (
    <SafeScreen>
      {/* HEADER */}
      <View className="px-6 pb-5 border-b border-gray-100/60 flex-row items-center">
        <TouchableOpacity onPress={() => router.back()} className="mr-4">
          <Ionicons name="arrow-back" size={25} color="#111" />
        </TouchableOpacity>
        <Text className="text-black text-2xl font-bold">Privacy & Security</Text>
      </View>
      ...
      >
        ...
        <View className="px-6 pt-6">
          <Text className="text-black text-lg font-bold mb-4">Security</Text>

          {securitySettings.map((setting) => (
            <TouchableOpacity
              key={setting.id}
              className="bg-gray-100/60 rounded-2xl p-4 mb-3"
            ...
            >
              <View className="flex-row items-center">
                <View className="bg-primary/20 rounded-full w-12 h-12 items-center justify-center mr-4">
                  <Ionicons name={setting.icon as any} size={24} color="#111" />
                </View>
                ...
                {setting.type === "toggle" ? (
                  <Switch
                    ...
                    thumbColor="#FFFFFF"
                    trackColor={{ false: "#2A2A2A", true: "yellow" }}
        ...
        <View className="px-6 pt-4">
          <Text className="text-black text-lg font-bold mb-4">Privacy</Text>

          {privacySettings.map((setting) => (
            <View key={setting.id}>
              <View className="bg-gray-100/60 rounded-2xl p-4 mb-3">
                <View className="flex-row items-center">
                  <View className="bg-primary/20 rounded-full w-12 h-12 items-center justify-center mr-4">
                    <Ionicons name={setting.icon as any} size={24} color="#111" />
                  </View>
        ...
        {/* ACCOUNT SECTION */}
        <View className="px-6 pt-4">
          <Text className="text-black text-lg font-bold mb-4">Account</Text>
        ...
        <View className="px-6 pt-4">
          <TouchableOpacity
            className="bg-gray-100/60 rounded-2xl p-5 flex-row items-center justify-between border-2 border-red-500/20"
            activeOpacity={0.7}
          >
            <View className="flex-row items-center">
