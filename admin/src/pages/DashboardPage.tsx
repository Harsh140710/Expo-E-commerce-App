                            ...
                            <table className="table">
                                ...
                                <tbody>
                                    {recentOrders.map((e:any) => (
                                      <tr key={e._id}>
                                        <td>
                                          <span className="font-medium">#{e._id.slice(-8).toUpperCase()}</span>
                                        </td>

                                        <td>
                                          <div className="font-medium">{e.shippingAddress.fullName}</div>
                                          <div className="text-sm opacity-60">{e.orderItems.length} item(s)</div>
                                        </td>

                                        <td>
                                          <div className="text-sm">
                                            {e.orderItems[0]?.name}
                                            {e.orderItems.length > 1 && ` +${e.orderItems.length - 1} more`}
                                          </div>
                                        </td>

                                        <td>
                                          <span className="font-medium">${e.totalPrice.toFixed(2)}</span>
                                        </td>

                                        <td>
                                          <div className={`badge ${getOrderStatusBadge(e.status)}`}>
                                              {capitalizeText(e.status)}
                                          </div>
                                        </td>

                                        <td>
                                          <span className="text-sm opacity-60">{formatDate(e.createdAt)}</span>
                                        </td>
                                      </tr>
                                    ))}
                                </tbody>
